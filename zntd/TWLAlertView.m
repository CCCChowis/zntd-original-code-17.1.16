...
- (void)layoutSubviews
{
    ...
    [self createBtnTitle:_btnTitleArr :_parengBtnTag];
}
...
- (void)createBtnTitle:(NSArray *)titleArr :(int )parentBtnTag
{
...
//            [btn setBackgroundColor:[UIColor colorWithHexString:[ThemeSingleton sharedInstance].UINavgationBar alpha:1]];
        ...
        if ([titleArr[i] isEqualToString:@"种植"]){
            btn.tag=parentBtnTag;
        [btn addTarget:self action:@selector(clickToPlant:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)clickToPlant:(id)sender{
    
    UIButton * clickedBtn = sender;

    
    //页面跳转到首页，顺便把花的key值传过去

    UIViewController* viewcontroller=[self topViewController];
    UIStoryboard *storyboard = viewcontroller.storyboard;
    ViewController *plantView = [storyboard instantiateViewControllerWithIdentifier:@"view1"];
    plantView.transistFId = clickedBtn.tag;
    [viewcontroller presentViewController:plantView animated:YES completion:nil];
...
-(void)initWithTitle:(NSString *) title contentStr:(NSString *)content type:(NSInteger)type btnNum:(NSInteger)btnNum btntitleArr:(NSArray *)btnTitleArr parentBtntag:(NSInteger)parentBtnTag parentVC:(UIViewController *)parentVC
